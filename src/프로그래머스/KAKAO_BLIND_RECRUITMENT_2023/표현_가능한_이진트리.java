...
//            System.out.println(binary);
            boolean result = search(0, binary.length() - 1, binary);
            answer[i] = result ? 1 : 0;
        }
    ...
    private boolean search(int left, int right, String binary) {
        if (left == right) {
            return true;
        }
        ...
        // 왼쪽 오른쪽
        return search(left, mid - 1, binary) && search(mid + 1, right, binary);
    }
